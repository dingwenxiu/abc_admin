...
@Component({
  selector: 'app-report-agent-salary',
  templateUrl: './report-agent-salary.component.html',
  styleUrls: ['./report-agent-salary.component.scss']
})
export class ReportAgentSalaryComponent implements OnInit {
  public tableList = [];
  ...
  public async getDataList() {
    ...
    const response = await this.api.Http({name: 'salaryLists', data: {...this.searchData}});
    if (response.success) {
